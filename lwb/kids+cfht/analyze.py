...
for i in range(4):
    ...
    result = Table.read(os.path.join(
        'results', 'cfht_{}_nstar_low.csv'.format(i)))
    ax.errorbar(rp, rp * result['ds'], yerr=rp * result['ds_err'],
                color='magenta', marker='o', ls='', fillstyle='none',
                label=r'low $n_{\rm star}$')

    result = Table.read(os.path.join(
        'results', 'cfht_{}_nstar_high.csv'.format(i)))
    ax.errorbar(rp * 1.1, rp * result['ds'], yerr=rp * result['ds_err'],
                color='royalblue', marker='x', ls='', fillstyle='none',
                label=r'high $n_{\rm star}$')
...
plt.tight_layout(pad=0.8)
plt.subplots_adjust(wspace=0, hspace=0)
plt.savefig(os.path.join('results', 'cfht_nstar.pdf'.format(survey)))
plt.savefig(os.path.join('results', 'cfht_nstar.png'.format(survey)), dpi=300)
plt.close()
