...
for gals in ['bgs_any', 'lrg']:
    for part in ['north', 'south']:
        table_l = vstack([table_l, hstack([
            Table.read('dr9_sv3_{}_{}_0.57.0_basic.fits'.format(gals, part)),
            Table.read('dr9_sv3_{}_{}_0.57.0_pz.fits'.format(gals, part))])])
...
if args.survey.lower() == 'hsc':
    ...
    table_s = Table.read(os.path.join(
        zebu.base_dir, 'lwb', 'hsc', 'raw', 'hsc_s16a_lensing.fits')).filled()
    ...
    table_c_1 = vstack([
        ...
        Table.read(os.path.join(zebu.base_dir, 'lwb', 'hsc', 'raw',
                                'pdf-s17a_wide-9813.cat.fits'))]).filled()
    ...
    table_c_2 = Table.read(os.path.join(
        ...
        'Afterburner_reweighted_COSMOS_photoz_FDFC.fits')).filled()
    ...
    table_n['n'] = np.vstack([np.genfromtxt(fname.format(i + 1))[:, 1] for i
                              in range(5)]).T
...
for lens_bin in range(len(z_l_bins) - 1):
    ...
    add_precompute_results(table_l_part, table_s, rp_bins, **precompute_kwargs)
    ...
    add_continous_fields(table_l_part, distance_threshold=2)
    centers = jackknife_field_centers(table_l_part, 100)
    add_jackknife_fields(table_l_part, centers)
